...
impl WatchMap {
    ...
    pub fn cursor<'a>(
        &'a mut self,
        watches: &'a mut Mapping<ClauseId, WatchedLiterals>,
        literal: Literal,
    ) -> Option<WatchMapCursor<'a>> {
        let clause_id = *self.map.get(literal)?;
        let watched_literal = watches.get(clause_id).expect("no watches found for clause");
...
impl<'a> WatchMapCursor<'a> {
    ...
    pub fn watched_literals(&self) -> &WatchedLiterals {
        ...
        unsafe { self.watches.get_unchecked(self.current.clause_id) }
    }
    ...
    pub fn update(mut self, new_watch: Literal) -> Option<Self> {
        ...
        let clause_id = self.current.clause_id;
        ...
        if let Some(previous) = &self.previous {
            ...
            let previous_watches = unsafe { self.watches.get_unchecked_mut(previous.clause_id) };
        ...
        let watch = unsafe { self.watches.get_unchecked_mut(clause_id) };
